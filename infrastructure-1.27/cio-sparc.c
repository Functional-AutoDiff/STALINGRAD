...
#endif
{
	...
	*((VOIDP*)&(new_action.sa_handler)) = handler;
	sigemptyset (& (new_action.sa_mask));
	(new_action.sa_flags) = 0;
	sigaction (sig, (&new_action), (&old_action));
	return *((VOIDP*)&(old_action.sa_handler));
}
...
S2CUINT  sc_mutex = 0;			/* Mutual exclusion flag */

S2CUINT  sc_pendingsignals = 0;		/* pending signal mask */
