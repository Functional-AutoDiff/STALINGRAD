...
(define (fill-union-values! v us) (set-union-values! v us))
...
(define (zero v)
 ;; This is written in CPS so as not to break structure sharing.
 (let loop ((v v) (cs '()) (k (lambda (v cs) v)))
  (let ((found? (assq v cs)))
   (cond
     ...
     (let ((u (make-reverse-tagged-value 'unfilled)))
      (loop (reverse-tagged-value-primal v)
	    ...
	    (lambda (v cs)
	     ...
	     (k u cs)))))
    ((tagged-pair? v)
     (let ((u (make-tagged-pair (tagged-pair-tags v) 'unfilled 'unfilled)))
     ...
     (let ((u (make-recursive-closure
      ...
      (map2-cps loop
		...
		(lambda (vs cs)
		 ...
		 (k u cs)))))
    ((and (perturbation-tagged-value? v1) (perturbation-tagged-value? v2))
     (let ((u (new-perturbation-tagged-value 'unfilled)))
      (loop (perturbation-tagged-value-primal v1)
	    (perturbation-tagged-value-primal v2)
	    (cons (cons (cons v1 v2) u) cs)
	    (lambda (v cs)
	     ...
	     (k u cs)))))
    ((and (bundle? v1) (bundle? v2))
     (let ((u (new-bundle 'unfilled 'unfilled)))
      (loop (bundle-primal v1)
	    (bundle-primal v2)
	    (cons (cons (cons v1 v2) u) cs)
	    (lambda (v-primal cs)
	     (loop (bundle-tangent v1)
		   ...
		   (lambda (v-tangent cs)
		    ...
		    (k u cs)))))))
    ((and (sensitivity-tagged-value? v1) (sensitivity-tagged-value? v2))
     (let ((u (new-sensitivity-tagged-value 'unfilled)))
      (loop (sensitivity-tagged-value-primal v1)
	    (sensitivity-tagged-value-primal v2)
	    (cons (cons (cons v1 v2) u) cs)
	    (lambda (v cs)
	     ...
	     (k u cs)))))
    ((and (reverse-tagged-value? v1) (reverse-tagged-value? v2))
     (let ((u (new-reverse-tagged-value 'unfilled)))
      (loop (reverse-tagged-value-primal v1)
	    (reverse-tagged-value-primal v2)
	    (cons (cons (cons v1 v2) u) cs)
	    (lambda (v cs)
	     ...
	     (case (length (recursive-closure-variables v))
	      ...
	      ((1) `(let ,(map (lambda (x v) `(,x ,(loop v quote? vs)))
		     ...
		     (letrec ,(vector->list
		      ...
		      ,(vector-ref (recursive-closure-procedure-variables v)
				   (recursive-closure-index v)))))
	      (else `(let ,(map (lambda (x i) `(,x ,(loop v quote? vs)))
				(recursive-closure-variables v)
				(recursive-closure-values v))
		      (letrec ,(vector->list
				(map-vector
...
(define (move-values-up-value v additions)
 ;; returns: (list v additions)
 (assert (and (union? v) (not (null? additions))))
 (let ((v-new (abstract-value-union
	       v
	       (map-reduce abstract-value-union
			   (empty-abstract-value)
			   decrement-free-ups
			   (first additions)))))
  (list v-new
...
(define (reduce-depth path v1 v2)
 ...
 (let ((v-additions
	(let loop ((path path) (vs-above '()))
	 ...
	 (if (eq? (first path) v2)
	     (create-v-additions v1 (first path))
	     (let* ((v-additions
