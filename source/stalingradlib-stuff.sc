...
(define (new-nonrecursive-closure vs e)
 (assert
  (and (or (not *abstract?*) (every union? vs))
...
(define (new-recursive-closure vs xs es i)
 (assert
  (and
   (or (not *abstract?*) (every union? vs))
   (= (length vs)
...
(define (fill-recursive-closure-values! u vs)
 (assert
  (and
   (or (not *abstract?*) (every union? vs))
   (= (length vs) (length (recursive-closure-variables u)))
   ;; See the note in new-nonrecursive-closure. While that hasn't happened in
   ;; practise, and I don't know whether it can, I removed it in principle.
   (or *abstract?*
...
(define (new-bundle v v-perturbation)
 (assert (and (or (not *abstract?*) (and (union? v) (union? v-perturbation)))
...
(define (fill-bundle! u v v-perturbation)
 (assert (and (or (not *abstract?*) (and (union? v) (union? v-perturbation)))
	      ...
	      (not (empty-abstract-value? v))
	      (not (empty-abstract-value? v-perturbation))
	      (eq? (bundle-primal u) 'unfilled)
...
(define (fill-sensitivity-tagged-value-primal! u v)
 (assert (and (or (not *abstract?*) (union? v))
	      (not (empty-abstract-value? v))
	      (eq? (sensitivity-tagged-value-primal u) 'unfilled)))
...
(define (fill-reverse-tagged-value-primal! u v)
 (assert (and (or (not *abstract?*) (union? v))
	      (not (empty-abstract-value? v))
	      (eq? (reverse-tagged-value-primal u) 'unfilled)))
...
(define (new-tagged-pair tags v1 v2)
 (assert (and (or (not *abstract?*) (and (union? v1) (union? v2)))
	      (or *abstract?*
...
(define (fill-tagged-pair! u v1 v2)
 (assert (and
	  (or (not *abstract?*) (and (union? v1) (union? v2)))
	  (or *abstract?*
	  ...
	  (or (not *abstract?*)
	      (and (some-value-tags
		    (lambda (tags1) (prefix-tags? (tagged-pair-tags u) tags1))
		    v1)
		   (some-value-tags
		    (lambda (tags2) (prefix-tags? (tagged-pair-tags u) tags2))
		    v2)))
	  (eq? (tagged-pair-car u) 'unfilled)
...
(define (empty-abstract-value) (new-union '()))
...
(define (singleton v)
 (cond
  (*abstract?*
   (assert (or (not (union? v)) (empty-abstract-value? v)))
   (if (empty-abstract-value? v) v (new-union (list v))))
  (else (assert (not (union? v)))
	v)))
...
(define (some-value-tags p v)
 (let loop ((tags '()) (v v) (vs '()))
  (cond
   ...
   ((memq v vs) #t)
   ((union? v) (some (lambda (u) (loop tags u (cons v vs))) (union-values v)))
...
(define (every-value-tags p v)
 (let loop ((tags '()) (v v) (vs '()))
  (cond
   ;; needs work: I'm not sure that this is sound.
   ((memq v vs) #f)
   ((union? v) (every (lambda (u) (loop tags u (cons v vs))) (union-values v)))
   ((scalar-value? v) (p (reverse tags)))
   ((nonrecursive-closure? v)
    (p (append (reverse tags) (nonrecursive-closure-tags v))))
   ((recursive-closure? v)
...
(define (abstract-value-union v1 v2)
 (new-union (append (union-values v1) (union-values v2))))
      ...
      (let ((v-perturbation (make-union 'unfilled)))
       (map-cps loop
		...
		(lambda (us-perturbation cs)
		 ...
		 (k v-perturbation cs)))))
     ((vlad-empty-list? v)
      (let ((u-perturbation (new-perturbation-tagged-value (singleton v))))
      ...
      (let ((v-sensitivity (make-union 'unfilled)))
       (map-cps loop
		...
		(lambda (us-sensitivity cs)
		 ...
		 (k v-sensitivity cs)))))
     ((vlad-empty-list? v)
      (let ((u-sensitivity (new-sensitivity-tagged-value (singleton v))))
	     ...
	     (lambda (v-car-sensitivity cs)
	      (loop (tagged-pair-cdr v)
		    ...
		    (lambda (v-cdr-sensitivity cs)
		     (fill-tagged-pair! u-sensitivity
					v-car-sensitivity
					v-cdr-sensitivity)
		     (k u-sensitivity cs)))))))
     (else (internal-error)))))))
 ...
 (let loop ((v-sensitivity v-sensitivity) (cs '()) (k (lambda (r? cs) r?)))
  (let ((found? (memq v-sensitivity cs)))
   (cond
    (found? (k #t cs))
    ((union? v-sensitivity)
     (every-cps loop (union-values v-sensitivity) (cons v-sensitivity cs) k))
...
(define (debugging-externalize v)
 ...
 (let loop ((v v) (vs '()))
  (cond ((memq v vs) `(up ,(positionq v vs)))
	((union? v)
	 `(union ,@(map (lambda (u) (loop u (cons v vs))) (union-values v))))
	((vlad-empty-list? v) '())
	((vlad-true? v) #t)
	((vlad-false? v) #f)
	((real? v) v)
	  ...
	  ((union? v)
	   ...
	   (cond ((empty-abstract-value? v) 'bottom)
		 ((null? (rest (union-values v)))
		  (loop (first (union-values v)) quote? (cons v vs)))
...
(define (reduce-depth v v1 v2)
 ;; This is written in CPS so as not to break structure sharing.
 (assert (and (union? v) (union? v1) (union? v2)))
 (copy-abstract-value
...
(define (widen-abstract-value v)
 (assert (union? v))
 (limit-closure-depth (copy-abstract-value v)))
...
(define (debugging2 v)
 ...
 (let outer ((v v) (vs '()) (k (lambda (vs) #f)))
  (cond
   ((memq v vs) (k vs))
   ((union? v)
    (when (some (lambda (v-prime)
		 (and (union? v-prime) (abstract-value=? v-prime v)))
		vs)
     (format #t "Bingo ~s~%" (debugging-externalize v)))
    (let inner ((us (union-values v)) (vs (cons v vs)))
     (if (null? us)
	 (k vs)
	 (outer (first us) vs (lambda (vs) (inner (rest us) vs))))))
   ((scalar-value? v)
    (when (some (lambda (v-prime)
		 (and (not (union? v-prime)) (abstract-value=? v-prime v)))
		vs)
     (format #t "Ding ~s ~s~%" (debugging-externalize v)))
    (k (cons v vs)))
   (else
    (when (some (lambda (v-prime)
		 (and (not (union? v-prime)) (abstract-value=? v-prime v)))
		vs)
     (format #t "Dong ~s~%" (debugging-externalize v)))
	 ...
	 (when (or (and #f
			(some (lambda (u1)
			       (some (lambda (u2)
				     ...
				     (union-values v)))
			      (union-values v)))
		   (> (count-if perturbation-tagged-value? (union-values v)) 1)
		   (> (count-if bundle? (union-values v)) 1)
		   (> (count-if sensitivity-tagged-value? (union-values v)) 1)
		   (> (count-if reverse-tagged-value? (union-values v)) 1)
		   (some (lambda (u1)
			  (some (lambda (u2)
				 (and (tagged-pair? u1)
				      (tagged-pair? u2)
				      (not (eq? u1 u2))
				      (equal-tags? (tagged-pair-tags u1)
						   (tagged-pair-tags u2))))
				(union-values v)))
			 (union-values v)))
...
(define (debugging1)
 (for-each (lambda (e)
	    (for-each (lambda (b) (debugging2 (environment-binding-value b)))
		      (expression-environment-bindings e)))
	   *expressions*))
