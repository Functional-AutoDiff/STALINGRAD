...
void weight_gradient(adtl::adouble f(int, struct w_layer *), int n_ws_layers,
  ...
  for (i = 0; i<n_ws_layers; i++) {
	...
	for (i1 = 0; i1<n_ws_layers; i1++)
	  for (j1 = 0; j1<ws_layers[i1].n; j1++)
	    for (k1 = 0; k1<ws_layers[i1].w; k1++)
	      ws_layers[i1].layer[j1][k1] =
		ws_layers[i1].layer[j1][k1].getValue();
        ws_layers[i].layer[j][k].setADValue(1);
	result = f(n_ws_layers, ws_layers);
        grad_f[i].layer[j][k] = result.getADValue();}}}}
