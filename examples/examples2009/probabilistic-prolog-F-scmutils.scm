...
(define (map-indexed f l)
 (let loop ((l l) (i 0))
  (if (null? l) '() (cons (f (first l) i) (loop (rest l) (+ i 1))))))

(define (alpha-rename clause offset)
 (apply-substitution
  (map-indexed
   (lambda (variable i) (make-binding variable (make-variable (+ offset i))))
   (variables-in clause))
  clause))
...
(define (likelihood substitution-distribution)
 (map-reduce + 0.0 double-p substitution-distribution))
